...
const createMipmaps = canvas => {
  const mipmaps = [canvas]
  let curr = canvas
  while (curr.width > 38) {
    const size = Math.max(19, curr.width >> 1)
    const mip = document.createElement("canvas")
    mip.width = mip.height = size
    const ctx = mip.getContext("2d")
    ctx.imageSmoothingEnabled = true
    ctx.imageSmoothingQuality = "high"
    ctx.drawImage(curr, 0, 0, size, size)
    mipmaps.push(mip)
    curr = mip
  }
  return mipmaps
}
const getMipmap = (mipmaps, targetSize) => {
...
class SparkCellularAnimation {
  constructor(canvas) {
    ...
    this.cells = new Map()
    this.agents = []
    this.lastTick = this.ticks = this.doneAt = this.phase = this.velocity = this.position = this.lastTimestamp = 0
    ...
    this.maxScale = this.centerX = this.centerY = 0
  ...
  calculateSparkCells() {
    ...
    transformedPath.addPath(sparkPath, matrix)
    ctx.save()
    ctx.scale(152, 152)
    ctx.fillStyle = "#000"
      ...
      for (let x = 0; x < 19; x++) {
        ...
        for (let dy = 0; dy < 8; dy++) {
          for (let dx = 0; dx < 8; dx++) {
            if (imgData[((8 * y + dy) * 152 + (8 * x + dx)) * 4 + 3] > 8) count++
          }
        }
        if (count >= 11.52) {
          const idx = 19 * y + x
          cells.push({ idx, gx: x, gy: y, dist: Math.hypot(x - 9, y - 9) })
        }
      }
    }
    return cells.sort((a, b) => a.dist - b.dist || a.idx - b.idx)
  }
  ...
  calculateTotalSparkCells() {
    ...
    while (queue.length) {
      ...
      for (const n of this.neighborMap.get(curr) || []) {
  ...
  getGrowthCells(seed) {
    ...
    cells.forEach(c => addFrontier(c.gx, c.gy))
  ...
  reset() {
    ...
    this.lastTick = now
    this.ticks = this.doneAt = this.phase = this.velocity = 0
  ...
  update(timestamp) {
    ...
    if (this.doneAt && timestamp - this.doneAt >= 500 && !isFinal) {
      ...
      this.lastTick = now
      this.ticks = this.doneAt = 0
      this.phase++
    }
    if (isFinal && this.doneAt && timestamp - this.doneAt >= 2300 && Math.abs(this.velocity) < 1e-5) {
      this.stop()
      return false
    }
    return true
  ...
  updateCellularAutomaton(timestamp) {
    ...
    while (timestamp - this.lastTick >= 450) {
      ...
      for (const agent of this.agents) {
        if (agent.dieAt) {
          if (tickTime - agent.dieAt < 350) newAgents.push(agent)
          continue
        }
        this.cells.set(agent.idx, tickTime)
        const neighbors = (this.neighborMap.get(agent.idx) || [])
          .filter(n => !this.cells.has(n) && !aliveSet.has(n))
        if (!neighbors.length) {
  ...
  getAvailableCells() {
    ...
    for (const cellIdx of this.cells.keys()) {
      for (const n of this.neighborMap.get(cellIdx) || []) {
        if (!this.cells.has(n) && !visited.has(n)) {
          visited.add(n)
          available.push(n)
        }
      }
  ...
  draw(timestamp) {
    const { ctx, width, height, maxScale, phase, position, centerX, centerY, dpr } = this
    const scale = maxScale * Math.exp(phase * Math.log(19 / 0.78) - position)
  ...
  drawGrid(scale) {
    ...
    const drawDots = gridSize => {
      if (gridSize < 2 || (width / gridSize + 2) * (height / gridSize + 2) > 12000) return
      const alpha = Math.max(0, Math.min(1, (gridSize - 2) / 7)) * (0.45 + 0.45 * Math.min(1, gridSize / 90))
      if (alpha < 0.01) return
      ctx.fillStyle = "#C9C6BC"
      ctx.globalAlpha = alpha
      const startX = ((centerX + gridSize / 2) % gridSize + gridSize) % gridSize
      const startY = ((centerY + gridSize / 2) % gridSize + gridSize) % gridSize
  ...
  drawCells(timestamp, scale) {
    const { ctx, cells, cellImages, centerX, centerY, dpr } = this
    const cellScale = scale / 19
    if (cellScale < 0.4) return
    const r = 2 * cellScale * 0.78
    for (const [cellIdx, cellTime] of cells) {
      const gx = cellIdx % 19, gy = Math.floor(cellIdx / 19)
      const x = centerX + 2 * cellScale * (gx - 9)
      const y = centerY + 2 * cellScale * (gy - 9)
      const age = timestamp - cellTime
      const o = cellImages[cellIdx % 8]
      if (age < 700) {
        ...
        if (fadeProgress < 1) {
          ...
          ctx.drawImage(getMipmap(o.full, r * dpr), x - r / 2, y - r / 2, r, r)
          ctx.globalAlpha = 1
        }
        if (fadeProgress > 0) {
  ...
  drawAgents(timestamp, scale) {
    const { ctx, agents, fractalSolidMipmaps, centerX, centerY, dpr } = this
    const cellScale = scale / 19
    if (cellScale < 0.4) return
    for (const agent of agents) {
      const progress = Math.min(1, (timestamp - agent.since) / 450)
      const eased = 1 - (1 - progress) * (1 - progress)
      const prevX = centerX + 2 * cellScale * (agent.prev % 19 - 9)
      const prevY = centerY + 2 * cellScale * (Math.floor(agent.prev / 19) - 9)
      const currX = centerX + 2 * cellScale * (agent.idx % 19 - 9)
      const currY = centerY + 2 * cellScale * (Math.floor(agent.idx / 19) - 9)
      const x = prevX + (currX - prevX) * eased
      const y = prevY + (currY - prevY) * eased
      let opacity = agent.dieAt
        ? (1 - Math.min(1, (timestamp - agent.dieAt) / 350)) ** 2
        : 1 - (1 - Math.min(1, (timestamp - agent.bornAt) / 350)) ** 3
      if (opacity <= 0) continue
      const u = cellScale * opacity
      const size = 2 * u * 0.78
      if (size < 0.4) continue
      ctx.drawImage(getMipmap(fractalSolidMipmaps, 2 * size * 0.78 * dpr), x - size / 2, y - size / 2, size, size)
    }
  }
...
onMounted(() => {
  ...
  if (canvas) {
    animationInstance = new SparkCellularAnimation(canvas)
